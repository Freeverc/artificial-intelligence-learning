...
def saveResultImg(mapsize, pos_sn, pos_en, blocklist, routelist, idx):
    # 初始化导入的Pygame模块
    pygame.init()
    # 此处要将地图投影大小转换为像素大小，此处设地图中每个单元格的大小为CELL_WIDTH*CELL_HEIGHT像素
    mymap = Map((mapsize[0]*CELL_WIDTH, mapsize[1]*CELL_HEIGHT))
...
def runPathPlanning(mapsize, pos_snode, pos_enode, blocklist):
    myAstar = AStar(mapsize, pos_snode, pos_enode)
    myAstar.setBlock(blocklist)
...
def main():
    ...
    for testIdx in range(testNum):
        ...
        result=runPathPlanning(mapsize, pos_snode, pos_enode, blocklist)
